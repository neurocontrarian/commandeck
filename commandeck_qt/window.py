...
class CommandeckWindow(QMainWindow):
    ...
    def _build_toolbar(self):
        ...
        menu.addAction(_("Install Button Pack…"), self._on_install_pack)
        menu.addSeparator()
    ...
    def _update_category_bar(self, buttons: list[CommandButton], hidden_cats: set):
        ...
        self._cat_combo_action.setVisible(bool(cats))
    ...
    def _machine_display_name(self, machine_id) -> str:
        ...
        try:
            machine = next(
                (m for m in self._config.load_machines() if m.id == machine_id), None)
        except Exception:
            machine = None
    ...
    def _on_install_pack(self):
        if not self._require_pro(
            _("Button Packs require Commandeck Pro.\n\n"
              "Upgrade to install ready-made sets of commands "
              "(Linux / macOS / Windows) on your remote machines.")):
            return
        from commandeck_qt.dialogs.pack_install_dialog import show_pack_install_dialog
        show_pack_install_dialog(self._config, parent=self)
    ...
    def _on_sel_change_machine(self):
        ...
        machine = None if idx == 0 else machines[idx - 1]
        ...
        for btn in buttons:
            if btn.id not in ids:
                continue
            if machine is None:
                btn.machine_ids = []  # Local — always allowed
            elif os_compatible(btn.os, machine.os):
                btn.machine_ids = [machine.id]
            else:
                skipped += 1  # don't run a Linux command on a Windows host, etc.
        self._config.save_buttons(buttons)
        self._clear_selection()
        self.populate_grid()
        if skipped:

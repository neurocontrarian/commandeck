...
class MachinesListDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        tb = QToolBar()
        tb.setMovable(False)
        # Tactile press feedback: on press the button darkens and its content nudges
        # down-right, so it feels like a real key going down. palette() roles keep
        # it correct in both light and dark themes.
        tb.setStyleSheet(
            "QToolButton { border-radius: 8px; padding: 5px 12px; }"
        ...
        detect_action = tb.addAction(_("Detect"))
    ...
    def _refresh(self):
        ...
        for m in machines_sorted:
            ...
            from commandeck_core.utils.os_match import os_label
            badge = os_label(m.os)
            text = f"{m.name}  –  {m.user}@{m.host}:{m.port}"
            if badge:
                text += f"   ·  {badge}"
    ...
    def _on_detect(self):
        from commandeck_qt.dialogs.network_scan_dialog import show_network_scan_dialog
        show_network_scan_dialog(self._config, on_machine_added=self._refresh,
                                 parent=None)

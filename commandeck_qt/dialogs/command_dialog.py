...
from commandeck_core.utils.color import is_hex_color
...
class CommandDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self._refresh_machine_compat()  # initial compat state (no-op for cross-platform)
    ...
    def _build_details_group(self) -> QGroupBox:
        ...
        insert_var = QPushButton(_("Insert variable"))
        insert_var.setToolTip(_("Insert a {{variable}} (asked at run time) or a saved value"))
        insert_var.clicked.connect(self._on_insert_variable)
        cmd_row.addWidget(insert_var)
        cmd_wrap = QWidget()
        cmd_wrap.setLayout(cmd_row)
        form.addRow(_("Command"), cmd_wrap)
        return box

    def _on_insert_variable(self):
        """Insert a `{{variable}}` (resolved at run time) or freeze a saved value."""
        from commandeck_core.models.pack_variables import standard_keys
        saved = self._config.load_variable_values()
        dlg = QDialog(self)
        dlg.setWindowTitle(_("Insert variable"))
        v = QVBoxLayout(dlg)
        v.addWidget(QLabel(_("Insert it as {{variable}} (asked each run), or pick a "
                             "saved value to freeze it now.")))
        row = QHBoxLayout()
        row.addWidget(QLabel(_("Variable")))
        combo = QComboBox()
        combo.addItems(standard_keys())
        ...
        def refresh():
            lst.clear()
            for val in saved.get(combo.currentText().strip(), []):
                lst.addItem(val)
        combo.currentTextChanged.connect(lambda _t: refresh())
        refresh()

        chosen = {"text": None}
        btns = QHBoxLayout()
        ins_var = QPushButton(_("Insert {{variable}}"))
        ins_val = QPushButton(_("Insert selected value"))
        ...
        def do_val():
            ...
            if it is not None:
                chosen["text"] = it.text()
                dlg.accept()
        ins_var.clicked.connect(do_var)
        ins_val.clicked.connect(do_val)
        btns.addWidget(ins_var)
        btns.addWidget(ins_val)
        btns.addStretch(1)
        v.addLayout(btns)
        ...
        if dlg.exec() == QDialog.Accepted and chosen["text"]:
            self._command_edit.insert(chosen["text"])
    ...
    def _build_machines_group(self) -> QGroupBox:
        ...
        self._machine_by_id = {m.id: m for m in machines}
        if machines:
            ...
            for m in machines:
                cb = QCheckBox(self._machine_label(m, incompatible=False))
    ...
    def _on_all_toggled(self, checked: bool):
        ...
        for _, cb in self._machine_switches:
            if checked and not cb.isEnabled():
                continue  # "All" means all OS-compatible machines
            cb.blockSignals(True)
            cb.setChecked(checked)
            cb.blockSignals(False)

    def _machine_label(self, m, incompatible: bool) -> str:
        base = f"{m.name}  –  {m.user}@{m.host}:{m.port}"
        if not incompatible:
            return base
        os_name = {"linux": "Linux", "macos": "macOS", "windows": "Windows"}.get(
            (m.os or "").lower(), m.os)
        return f"{base}   ⚠ {os_name}"

    def _refresh_machine_compat(self):
        """Disable + flag machines whose OS is incompatible with the button's
        Command OS. Already-selected machines stay enabled so they can be removed."""
        if not self._machine_switches:
            return
        cur_os = self._os_values[self._os_combo.currentIndex()]
        for mid, cb in self._machine_switches:
            m = self._machine_by_id.get(mid)
            if m is None:
                continue
            compatible = os_compatible(cur_os, m.os)
            if compatible:
                cb.setEnabled(True)
                cb.setText(self._machine_label(m, incompatible=False))
            elif cb.isChecked():
                cb.setEnabled(True)  # assigned but now mismatched — keep visible
                cb.setText(self._machine_label(m, incompatible=True))
            else:
                cb.setEnabled(False)
                cb.setText(self._machine_label(m, incompatible=True))
    ...
    def _build_organisation_group(self) -> QGroupBox:
        ...
        # OS the command is written for. Cross-platform = runs on any machine;
        # an OS-specific button only attaches to machines of that OS on propagation.
        self._os_values = ["", "linux", "macos", "windows"]
        self._os_combo = QComboBox()
        self._os_combo.addItems([_("Cross-platform"), "Linux", "macOS", "Windows"])
        self._os_combo.setToolTip(
            _("Which OS this command targets. Cross-platform runs anywhere; an "
              "OS-specific button only attaches to machines of that OS."))
        self._os_combo.currentIndexChanged.connect(self._refresh_machine_compat)
        form.addRow(_("Command OS"), self._os_combo)
        return box
    ...
    def _populate_fields(self):
        ...
        self._os_combo.setCurrentIndex(
            self._os_values.index(btn.os) if btn.os in self._os_values else 0)
    ...
    def _build_button_from_fields(self) -> CommandButton | None:
        ...
        if self._is_edit and self._button:
            ...
            b.os = self._os_values[self._os_combo.currentIndex()]
            ...
            btn = CommandButton(
                ...
                os=self._os_values[self._os_combo.currentIndex()],

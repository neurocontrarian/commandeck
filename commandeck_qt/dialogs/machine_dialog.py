...
class MachineDialog(QDialog):
    def __init__(self, config: ConfigManager, machine: Machine | None = None,
        ...
        # ``prefill`` (host/name/port from network detection) seeds an *Add* — it
        # never makes this an edit, so a fresh machine is saved via add_machine().
    ...
    def _build_connection_group(self) -> QGroupBox:
        ...
        # OS of this host — used to only attach OS-compatible buttons when you
        # propagate this machine to your buttons.
        self._os_values = ["", "linux", "macos", "windows"]
        self._os_combo = QComboBox()
        self._os_combo.addItems([_("Unknown"), "Linux", "macOS", "Windows"])
        self._os_combo.setToolTip(
            _("Only buttons written for this OS (or cross-platform ones) are "
              "attached when you add this machine to all buttons."))
        ...
        form.addRow(_("Operating System"), self._os_combo)
    ...
    def _populate_fields(self):
        if not self._machine:
            if self._prefill:
                if self._prefill.get("name"):
                    self._name_edit.setText(self._prefill["name"])
                if self._prefill.get("host"):
                    self._host_edit.setText(self._prefill["host"])
                if self._prefill.get("port"):
                    self._port_spin.setValue(int(self._prefill["port"]))
            return
        ...
        if self._machine.os in self._os_values:
            self._os_combo.setCurrentIndex(self._os_values.index(self._machine.os))
    ...
    def _build_machine_from_fields(self, for_test: bool = False) -> Machine | None:
        ...
        os_value = self._os_values[self._os_combo.currentIndex()]
        ...
        if for_test or not (self._is_edit and self._machine):
            machine = Machine(name=name, host=host, user=user, port=port,
                              ...
                              group=group, os=os_value)
        else:
            m = self._machine
            m.name, m.host, m.user, m.port = name, host, user, port
            m.identity_file = key
            m.icon_name, m.group, m.os = self._icon_name, group, os_value
            machine = m
    ...
    def _on_save(self):
        ...
        # Changing a machine's OS can leave buttons targeting it that it can no
        # longer run — detach those so the grid stays honest.
        pruned = self._config.prune_incompatible_machine_links()
        if pruned:
            QMessageBox.information(
                self, _("Machine OS changed"),
                _("Detached this machine from {n} button(s) now incompatible "
                  "with its OS.").format(n=pruned))

...
class PreferencesDialog(QDialog):
    ...
    def _rebuild_license_section(self):
        ...
        if info.get("active"):
            ...
            if is_yearly:
                ...
                # No lifetime plan is sold; a non-yearly active license is a
                # legacy / no-expiry key. Show the brand without an expiry line.
    ...
    def _build_backup_tab(self) -> QWidget:
        ...
        # Buttons & settings backup. Escape the "&" so Qt doesn't treat it as a
        # mnemonic accelerator in the group-box title (it would swallow the next
        # char / show a stray underline). "&&" renders a literal ampersand.
        btn_box = QGroupBox(_("Buttons & Settings Backup").replace("&", "&&"))
        btn_box.setToolTip(
            _("Export your buttons and settings to a .cdbackup file.\n"
              "Machines are exported separately below.")
        )
        btn_vbox = QVBoxLayout(btn_box)
        exp_btn = QPushButton(_("Export buttons & settings…").replace("&", "&&"))
        exp_btn.clicked.connect(self._on_export_buttons)
        imp_btn = QPushButton(_("Import buttons & settings…").replace("&", "&&"))

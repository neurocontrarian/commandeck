...
class PreferencesDialog(QDialog):
    ...
    def _build_desktop_tab(self) -> QWidget:
        ...
        # Preferred terminal for terminal-mode buttons. "Automatic" (empty value)
        # lets the executor pick the first installed terminal; this lets the user
        # override when their distro ships an unusual terminal.
        from commandeck_core.services.executor import available_terminals
        self._term_combo = QComboBox()
        self._term_combo.setEditable(True)
        self._term_combo.addItem(_("Automatic (detect)"), "")
        for _binname in available_terminals():
            self._term_combo.addItem(_binname, _binname)
        cur_term = self._settings.get_str("preferred-terminal")
        if cur_term:
            i = self._term_combo.findData(cur_term)
            if i < 0:
                self._term_combo.addItem(cur_term, cur_term)
                i = self._term_combo.count() - 1
            self._term_combo.setCurrentIndex(i)
        else:
            self._term_combo.setCurrentIndex(0)
        self._term_combo.setToolTip(
            _("Terminal used for buttons that open a terminal window. "
              "Leave on Automatic unless detection fails."))
        self._term_combo.currentIndexChanged.connect(self._on_terminal_changed)
        self._term_combo.lineEdit().editingFinished.connect(self._on_terminal_changed)
        desktop_form.addRow(_("Terminal"), self._term_combo)
    ...
    def _on_terminal_changed(self, *args):
        if self._term_combo.currentIndex() == 0:
            value = ""  # the "Automatic (detect)" item
        else:
            value = self._term_combo.currentData() or self._term_combo.currentText().strip()
        self._settings.set_str("preferred-terminal", value)
        # detect_terminal() is cached on its argument — clear so the new choice
        # takes effect immediately (also covers a freshly installed terminal).
        from commandeck_core.services.executor import detect_terminal, _resolve_terminal
    ...
    def _rebuild_license_section(self):
        ...
        if info.get("active"):
            ...
            if is_yearly:
                ...
                # One-time purchase (expires_at=null → type='lifetime').
                # Show the brand without an expiry line.
                title = "Commandeck Pro"
                sub = ""
            ...
            if is_yearly:
                renew_btn = QPushButton(_("Renew license — $29 one-time"))
            ...
            form.addRow(
                _("Status"),
                QLabel(_("Free tier — upgrade to Pro for SSH, themes and AI."))
            )
            buy_btn = QPushButton(_("Get Commandeck Pro — $29 one-time"))
    ...
    def _build_backup_tab(self) -> QWidget:
        ...
        # Buttons backup (buttons-only .cdbackup; machines exported separately below)
        btn_box = QGroupBox(_("Buttons Backup"))
        btn_box.setToolTip(
            _("Export your buttons to a .cdbackup file.\n"
              "Machines are exported separately below.")
        )
        btn_vbox = QVBoxLayout(btn_box)
        exp_btn = QPushButton(_("Export buttons…"))
        exp_btn.clicked.connect(self._on_export_buttons)
        imp_btn = QPushButton(_("Import buttons…"))
        ...
        # Variables backup (saved {{variable}} values — kept in a separate file)
        var_box = QGroupBox(_("Variables Backup"))
        var_box.setToolTip(
            _("Export your saved variable values ({{variables}}).\n"
              "Kept in a separate file from buttons and machines.")
        )
        var_vbox = QVBoxLayout(var_box)
        exp_v_btn = QPushButton(_("Export variables…"))
        exp_v_btn.clicked.connect(self._on_export_variables)
        imp_v_btn = QPushButton(_("Import variables…"))
        imp_v_btn.clicked.connect(self._on_import_variables)
        var_vbox.addWidget(exp_v_btn)
        var_vbox.addWidget(imp_v_btn)
        vbox.addWidget(var_box)
    ...
    @staticmethod
    def _license_key_safe() -> str:
        """Local license key (the personal-backup MAC secret), or '' if no Pro/key.

        Lazily imported so the free binary — which contains zero Pro code — never
        hard-depends on the license module."""
        try:
            from commandeck_core.pro.license import get_license_key
    ...
    def _on_export_buttons(self):
        path = _safe_get_save_filename(
            self, _("Export buttons"),
            "commandeck.cdbackup", "Commandeck backup (*.cdbackup)",
        )
        if path:
            try:
                self._config.export_backup(Path(path), sign_key=self._license_key_safe())
    ...
    def _on_import_buttons(self):
        path = _safe_get_open_filename(
            self, _("Import buttons"),
            "Commandeck backup (*.cdbackup *.rxbackup)",
        )
        if not path:
            return
        # Recognise the user's own backup (signed by this license) — those import
        # without friction. An unsigned / foreign / tampered backup is treated as
        # untrusted: show every command and an explicit warning before importing,
        # since a button can run any command on this computer.
        try:
            peek = self._config.peek_backup(Path(path), verify_key=self._license_key_safe())
        except Exception as e:
            QMessageBox.critical(self, _("Import failed"), str(e))
            return
        if not peek.get('personal') and not self._confirm_unknown_backup(peek.get('buttons', [])):
            return
        mode = self._ask_import_mode()
        if mode is None:
            return
        try:
            need_pwd = self._config.import_backup(Path(path), mode=mode)
    ...
    def _ask_import_mode(self):
        """Ask whether to replace all buttons or only add the new ones.
        Returns "replace", "merge", or None if the user cancelled."""
        box = QMessageBox(self)
        box.setWindowTitle(_("Import buttons"))
        box.setText(_("How should these buttons be imported?"))
        box.setInformativeText(
            _("Replace all: remove yours, use the file's.") + "\n"
            + _("Add new: keep yours, add the rest."))
        replace_btn = box.addButton(_("Replace all"), QMessageBox.DestructiveRole)
        merge_btn = box.addButton(_("Add new"), QMessageBox.AcceptRole)
    ...
    def _confirm_unknown_backup(self, buttons) -> bool:
        """Show every command in an unknown-source backup + a warning, and ask the
        user to confirm. Returns True to proceed, False to abort. Mirrors the pack
        gallery's command preview so a relabelled malicious file can't sneak
        ready-to-click commands in unseen."""
        from PySide6.QtWidgets import QDialog, QVBoxLayout, QTextEdit, QDialogButtonBox
        dlg = QDialog(self)
        dlg.setWindowTitle(_("Import buttons"))
        dlg.setMinimumWidth(520)
        lay = QVBoxLayout(dlg)
        warn = QLabel(
            _("⚠️ This backup is from an unknown source. A button can run any "
              "command on your computer. Review every command below and import "
        ...
        lines = []
        for b in buttons:
            lines.append(f"• {_(b.name)}")
            lines.append(f"    {b.command}")
        preview.setPlainText("\n".join(lines) or _("(no buttons)"))
        lay.addWidget(preview)
        bb = QDialogButtonBox()
        import_btn = bb.addButton(_("Import"), QDialogButtonBox.AcceptRole)
    ...
    def _on_export_variables(self):
        path = _safe_get_save_filename(
            self, _("Export variables"),
            "commandeck.cdvariables", "Commandeck variables (*.cdvariables)",
        )
        if path:
            try:
                self._config.export_variables_backup(Path(path))
                self._toast(_("Variables exported successfully"))
            except Exception as e:
                QMessageBox.critical(self, _("Export failed"), str(e))

    def _on_import_variables(self):
        path = _safe_get_open_filename(
            self, _("Import variables"),
            "Commandeck variables (*.cdvariables)",
        )
        if path:
            try:
                n = self._config.import_variables_backup(Path(path))
                self._toast(_("Imported saved values for {n} variable(s)").format(n=n))
            except Exception as e:
                QMessageBox.critical(self, _("Import failed"), str(e))

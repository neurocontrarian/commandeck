# PyInstaller spec for Commandeck Linux Qt AppImage (onedir build).
# Free build:  COMMANDECK_PRO=0 pyinstaller --noconfirm --clean packaging/linux/Commandeck.spec
# Pro build:   COMMANDECK_PRO=1 pyinstaller --noconfirm --clean packaging/linux/Commandeck.spec
#
# Run from repo root. Output: dist/Commandeck/ (onedir) — wrapped into an AppImage
# by packaging/linux/build_appimage.sh.
...
a = Analysis(
    ...
    datas=[
        ...
        # Third-party license notices (LGPL obligation for Qt/PySide6 + Paramiko)
    ...
    hiddenimports=[
        ...
        # SecretService backend pulls these; list explicitly so the AppImage can
        # actually reach the OS keychain (else secret_store uses the .secrets fallback).
        'secretstorage',
        'jeepney',
        'commandeck_core.platform.linux',
    ] + ([
        'cryptography',
        'bcrypt',
        'commandeck_core.pro',
...
# NOTE: No BUNDLE block — BUNDLE is macOS-only (.app). Linux uses the COLLECT
# onedir output (dist/Commandeck/) wrapped into an AppImage by build_appimage.sh.

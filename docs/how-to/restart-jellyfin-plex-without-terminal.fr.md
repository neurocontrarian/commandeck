...
!!! tip "Il existe un ensemble tout prêt pour Jellyfin"
    Cette page construit un bouton à la main, ce qui est la bonne façon d'apprendre.
    Si vous utilisez Jellyfin et voulez les treize autres — pourquoi ça charge, ce qui
    remplit le disque, le journal à lire — installez plutôt le pack :
    [Garder Jellyfin en forme sans apprendre Docker](../use-cases/jellyfin.fr.md).

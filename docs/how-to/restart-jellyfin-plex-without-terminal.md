...
!!! tip "There is a ready-made set for Jellyfin"
    This page builds one button by hand, which is the right way to learn. If you run
    Jellyfin and want the other thirteen — why it buffers, what fills the disk, the log
    to read — install the pack instead:
    [Keep Jellyfin Healthy Without Learning Docker](../use-cases/jellyfin.md).

...
!!! example "Éteindre une machine distante — le cas d'école"
    Utilisé via SSH (par exemple depuis votre téléphone), le bouton **Shutdown** par défaut échoue avec un message d'authentification. La commande est bonne : éteindre une machine demande des **droits administrateur**, et une connexion à distance ne les obtient pas automatiquement comme lorsque vous êtes assis devant l'ordinateur. (Sur votre propre PC, le même bouton marche directement.) Le bouton **Reboot** est dans le même cas.
    ...
    - assignez-le à **Shutdown** (et **Reboot**) ; laissez la commande inchangée

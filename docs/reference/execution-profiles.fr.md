...
!!! example "Éteindre une machine distante — le cas d'école"
    Utilisé via SSH (par exemple depuis votre téléphone), le bouton **Éteindre** par défaut échoue avec un message d'authentification. La commande est bonne : éteindre une machine demande des **droits administrateur**, et une connexion à distance ne les obtient pas automatiquement comme lorsque vous êtes assis devant l'ordinateur. (Sur votre propre PC, le même bouton marche directement.) Le bouton **Redémarrer** est dans le même cas.

    La solution est un profil, pas une autre commande :

    - créez un profil **Alimentation (admin)** → *Exécuter en tant que* `root`, et renseignez le **Mot de passe sudo** (votre mot de passe sur cette machine)
    - assignez-le à **Éteindre** (et **Redémarrer**) ; laissez la commande inchangée

    Le bouton s'exécute alors avec les droits administrateur et éteint proprement la machine. Ce seul profil sert ensuite pour tout ce qui demande des droits admin sur une machine distante — redémarrer un service, installer un logiciel, monter un disque.

...
**Toutes les machines** — une option en haut de la liste des machines pour sélectionner/désélectionner toutes les machines en même temps (elle ne prend que les machines *compatibles au niveau de l'OS*).

### OS de la commande

**OS de la commande** indique à Commandeck pour quel système d'exploitation la commande est écrite : **Multiplateforme** (le défaut — fonctionne partout), **Linux**, **macOS** ou **Windows**. Cela fait deux choses :

- Dans la liste des machines ci-dessus, les machines dont l'OS ne correspond pas sont **grisées** avec un repère ⚠, pour éviter de pointer par erreur une commande Linux vers une machine Windows (et inversement). Une machine déjà sélectionnée reste visible afin que vous puissiez la retirer.
- À l'[import](../pro/backup.md), une version Linux et une version Windows du même bouton sont conservées côte à côte au lieu d'entrer en conflit.

L'OS de chaque machine se règle dans son [éditeur](ssh-machines.md) ; Linux et macOS sont considérés compatibles. Laissez un bouton en **Multiplateforme** sauf si sa commande est spécifique à un OS.

!!! tip "Fonctionnalité Pro"
    L'ajout de machines SSH nécessite [Commandeck Pro](../pro.md). Dans la version gratuite, seul **Local** est disponible.
...
Saisissez un nom de catégorie pour assigner ce bouton à un groupe. Les boutons partageant le même nom de catégorie sont regroupés sous la même entrée dans le [filtre de catégories](main-window.md#filtre-de-catégories).

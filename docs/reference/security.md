...
SSH connections (Pro) authenticate with an **SSH key** (recommended) or a **password**. Commandeck never keeps a plaintext password on disk: a stored SSH password lives in your operating system's secure keychain (GNOME Keyring / KWallet, macOS Keychain, Windows Credential Manager), encrypted at rest and **never included in a backup**. Where no system keychain exists, it falls back to a local obfuscated file readable only by your user, and warns you that this is not strong encryption.
...
A profile's optional [sudo password](execution-profiles.md) is **never stored in plaintext**. It is kept in your operating system's secure keychain — the config files hold only a boolean flag, never the secret itself, so a sudo password is **never included in a backup**. On a system without a keychain, Commandeck falls back to a local file XOR-obfuscated with your machine's unique ID (not portable to another machine) and warns you. At runtime the password is passed to `sudo -S` so no terminal prompt is needed.

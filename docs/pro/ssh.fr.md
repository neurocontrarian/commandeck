...
!!! tip "Fonctionnalité Pro"
    Les machines SSH et les boutons multi-machines nécessitent [Commandeck Pro](../pro.md).
...
3. Renseignez le nom de la machine, l'hôte, l'utilisateur SSH et le mode d'authentification — une **clé** SSH (recommandé) ou un **mot de passe**
4. Cliquez sur **Tester** pour vérifier la connexion
5. Cliquez sur **Enregistrer**

La machine est maintenant disponible dans chaque éditeur de bouton.

!!! note "Clé ou mot de passe"
    Les machines SSH s'authentifient avec une **clé** (recommandé) ou un **mot de passe**. Un mot de passe enregistré réside dans le trousseau de votre système — jamais en clair ni dans une sauvegarde. Voir [Mot de passe SSH](../reference/ssh-machines.md#mot-de-passe-ssh) pour les détails.
...
Le sélecteur de machines est à choix multiple : cochez plusieurs machines et Commandeck exécute la commande sur toutes en même temps. Les résultats s'ouvrent dans **une seule fenêtre avec un sélecteur de machine** (◀ ▶) pour passer d'une sortie à l'autre, avec un statut ✓ / ✗ par machine.
...
Les trois modes d'exécution fonctionnent via SSH. Pour les sessions SSH interactives, utilisez **Ouvrir dans le terminal** — Commandeck génère automatiquement la bonne invocation `ssh -t`.

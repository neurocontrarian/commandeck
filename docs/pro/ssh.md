...
!!! tip "Pro feature"
    SSH machines and multi-machine buttons require [Commandeck Pro](../pro.md).
...
3. Fill in the machine name, host, SSH user, and how to authenticate — an SSH **key** (recommended) or a **password**
4. Click **Test** to verify the connection
5. Click **Save**

The machine is now available in every Button Editor.

!!! note "Key or password"
    SSH machines authenticate with a **key** (recommended) or a **password**. A stored password lives in your OS keychain — never in plaintext or in a backup. See [SSH Password](../reference/ssh-machines.md#ssh-password) for details.
...
The machine picker is multi-select: tick several machines and Commandeck runs the command on all of them at once. The results open in a **single window with a machine selector** (◀ ▶) so you can switch between each machine's output, with a per-machine ✓ / ✗ status.
...
All three execution modes work over SSH. For interactive SSH sessions, use **Open in terminal** — Commandeck generates the correct `ssh -t` invocation automatically.

...
**Button Packs** — ready-made button sets from the public gallery

| Tool | Description |
|------|-------------|
| `list_packs` | List gallery packs, each with `installed` and `update_available` flags |
| `install_pack` | Install a pack onto machines (signature-verified packs only) |
| `update_pack` | Update an installed pack, keeping your edits, targets and positions |
| `uninstall_pack` | Remove every button from a pack |
| `export_pack` | Export selected buttons to a shareable `.cdpack` file |

**Variables**

| Tool | Description |
|------|-------------|
| `list_variable_values` | List your saved values for command `{{variables}}` |

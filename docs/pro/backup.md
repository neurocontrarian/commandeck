...
Commandeck provides three separate export formats — buttons, machines and saved variable values — intentionally kept apart for security reasons.
...
## Variables backup — `.cdvariables`

This file contains your **saved variable values** — the reusable answers for `{{variables}}` you manage from **Menu → Variable Values** (for example, a list of your container names). It does **not** contain buttons or machines.

### Exporting

Click **Export variables**. Choose a location and save the `.cdvariables` file.

### Importing

Click **Import variables** and select a `.cdvariables` file. Saved values are merged into yours — any value you don't already have for a variable is added.
...
6. Import the `.cdvariables` file to restore your saved variable values
7. Test each machine connection from **Menu → Manage Machines → (select machine) → Test**

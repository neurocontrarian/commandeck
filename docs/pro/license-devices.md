...
!!! tip "Pro feature"
    A license key is required to activate [Commandeck Pro](../pro.md).

Each license key allows **3 simultaneous activations on desktop devices** (Linux, macOS, Windows) that you personally own or control.

!!! note "Desktop only — Android is a separate product"
    This license covers the **desktop app** (Linux, macOS, Windows). The Android app is a separate product on Google Play with its own billing; it is not activated with a desktop license key and does not consume slots here.
...
2. Set up the new machine and install Commandeck
...
1. Open Commandeck on any other device that is still activated and deactivate from **Preferences → License → Deactivate** to free a slot, then retry
...
Open Commandeck on one of your 3 active devices, go to **Preferences → License → Deactivate**, then activate on the new device.

!!! note
    The LemonSqueezy customer portal does **not** allow self-service instance deactivation. To free a slot, use the **Deactivate** button inside Commandeck on one of your active devices.

---

## Key sharing policy

Each license key is personal and non-transferable. At activation, Commandeck verifies that the email you enter matches the purchase email on record. Keys detected on more than 3 devices simultaneously will be revoked without refund.

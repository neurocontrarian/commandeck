...
Votre email d'achat LemonSqueezy contient un lien **Files**. Cliquez dessus pour télécharger `Commandeck-Pro-VERSION-x86_64.AppImage`.
...
!!! info "Prérequis système"
    Aucun — l'AppImage Pro est autonome (Python, Qt et toutes les dépendances sont
    incluses). Si votre distribution signale un greffon de plateforme Qt manquant au
    lancement, installez `libxcb-cursor0` :
    ```bash
    # Ubuntu / Mint / Debian
...
L'AppImage Pro est autonome — toutes les bibliothèques SSH (Paramiko, cryptography, etc.) sont incluses. Aucun `pip install` nécessaire.
...
| Toutes les 24 heures | Commandeck revalide silencieusement votre licence en ligne (pas d'UI, aucun ralentissement) |
...
Après la première activation, Commandeck fonctionne hors-ligne. Nous revalidons seulement une fois toutes les 24 heures, avec 3 jours de grâce si le réseau est coupé.

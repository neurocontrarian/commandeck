...
The core is **free and open source (AGPLv3)**: unlimited local commands and buttons,
no account, no expiry. **Pro** is a **$29 one-time purchase** (no subscription; includes
1 year of updates) that adds SSH machines, multi-machine buttons, themes, backup/restore,
...
your own disk, and the core is AGPLv3, so the source stays available.

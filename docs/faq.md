...
and the MCP server. Every Pro build includes an automatic
**14-day trial** — no card, no email. See [Pro & pricing](pro.md).

## I bought Pro — how do I activate it?

Free and Pro are **two separate downloads**. Your license key activates in the **Pro**
edition: [download it](download.md), open **Preferences**, click **Activate Pro** and paste
your key. If you were using the free edition, don't worry — both editions share the same
configuration, so **your buttons and settings carry over** automatically.
...
plain TOML on your disk. Several people can still act on the same server: each one runs
Commandeck on their own device and keeps their own buttons for that machine — so everyone
under the same roof can drive the family server. The only real difference is where the
buttons live: on each person's computer or phone, not on a central server you have to set
up and maintain.

## Does it send any data out? Telemetry? Account?
...
for sharing useful buttons. Every report is read, and bugs get fixed.

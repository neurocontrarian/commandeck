...
The **Android** app is distributed through the **Google Play Store** as an auto-renewing **subscription**, with Google as the seller and payment processor (not LemonSqueezy). For the Android app, the following apply in place of the license-key terms in sections 2–5:

- Access requires an **active Google Play subscription** tied to your Google account. There is **no free tier, no license key and no device-count limit** on mobile.
- The subscription **renews automatically** until cancelled. You may **cancel at any time** in the Google Play Store to stop future renewals; access continues until the end of the current billing period. Auto-renewal, billing and cancellation are also governed by **Google Play's terms of service**.
- The open-source core of Commandeck remains licensed under the **AGPLv3** (source at [github.com/neurocontrarian/commandeck](https://github.com/neurocontrarian/commandeck)); the **Commandeck** name and branding remain reserved.
...
**Free tier.** The open-source core of Commandeck is free software licensed under the **GNU Affero General Public License, version 3 (AGPLv3)**. The source code is available at [github.com/neurocontrarian/commandeck](https://github.com/neurocontrarian/commandeck). You may use, study, modify, and distribute it under the terms of the AGPLv3; if you distribute a modified version, or run a modified version as a network service, you must make your modifications available under the AGPLv3.

**Trademarks.** No license granted here, including the AGPLv3, grants any right to the **Commandeck** name, logo, or other trademarks or branding of the author, which are reserved. You may not use them in a way that implies endorsement or affiliation without permission.
...
Pro licenses include 1 year of updates from the date of purchase. After the first year, the version of the Software installed during that period continues to work indefinitely — there is no expiry of Pro features for the version you have installed. Future updates may require a renewal (price and terms to be announced).

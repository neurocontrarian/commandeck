...
Group buttons into categories like *NAS*, *Pi*, *Docker* so the grid stays readable. Pick a category from the header dropdown to filter.
...
- **Mixed OS fleet:** today a button holds one command. If a target runs a different OS than the command expects (PowerShell vs bash), it may not run — keep OS-specific buttons separate for now.

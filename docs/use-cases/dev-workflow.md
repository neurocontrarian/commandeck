...
This guide shows how to use Commandeck as a developer's command palette. The goal: replace the most repetitive terminal commands with one-click buttons, organised by project.

!!! tip "Pro feature"
    This workflow runs commands on a remote VPS over SSH. **SSH machines, multi-machine buttons, and execution profiles require [Commandeck Pro](../pro.md).** The local-only buttons in this guide work on the free tier.
...
You run the same commands dozens of times a day: build, test, deploy, check logs. Commandeck replaces your muscle memory with visible, labelled buttons.
...
1. **Ctrl+click** each Backend button (or rubber-band select them all) — the action bar appears at the bottom
2. Click **Machine** in the action bar
...
Two clean categories — Frontend and Backend — in the header dropdown surface exactly the commands you need. Your terminal stays open for exploratory work; Commandeck handles the repetitive parts.

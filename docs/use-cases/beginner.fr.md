...
![La fenêtre principale de Commandeck, pleine de boutons prêts à l'emploi](../assets/main-window.png)

---

## Vous avez déjà 40 boutons

Au premier lancement de Commandeck, votre grille est remplie avec quatre catégories de boutons préconfigurés :
...
|--------|------------------|
| **Git Status** | État de l'arbre de travail — fichiers indexés, modifiés et non suivis (`git status`) |
| **Git Log** | Les 15 derniers commits au format compact (`git log --oneline`) |
| **Git Diff** | Modifications non indexées dans le dossier courant (`git diff`) |
| **Docker PS** | Conteneurs Docker en cours d'exécution |
| **Docker PS All** | Tous les conteneurs, y compris ceux arrêtés |
| **Docker Images** | Images Docker disponibles localement |
| **Docker Clean** | Supprime les conteneurs arrêtés, réseaux inutilisés et images orphelines |
| **Tail Syslog** | Les 50 dernières lignes du journal système |
| **Disk I/O** | Statistiques d'entrées/sorties disque |
| **Python Version** | Versions installées de Python 3 et pip |
| **Pip Outdated** | Paquets Python ayant une version plus récente disponible |
| **Node Version** | Versions installées de Node.js et npm |
| **NPM Outdated** | Paquets npm ayant une version plus récente (depuis un dossier de projet Node) |
...
2. Désactivez **Développement**

La catégorie et tous ses boutons disparaissent de la vue. Ils ne sont pas supprimés — vous pouvez les faire réapparaître à tout moment au même endroit.
...
!!! note
    La modification des boutons par défaut nécessite [Commandeck Pro](../pro.md). Dans la version gratuite, vous pouvez utiliser, masquer et supprimer les boutons par défaut, mais pas les modifier.
...
Vous disposez de **10 boutons personnalisés gratuits**. En voici un facile pour commencer :

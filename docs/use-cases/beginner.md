...
![The Commandeck main window, full of ready-to-use buttons](../assets/main-window.png)

---

## You already have 40 buttons

The first time Commandeck launches, it populates your grid with four categories of pre-built buttons:
...
| Button | What it shows you |
|--------|-------------------|
| **Git Status** | Working tree status — staged, unstaged and untracked files (`git status`) |
| **Git Log** | The last 15 commits in compact one-line form (`git log --oneline`) |
| **Git Diff** | Unstaged changes in the current folder (`git diff`) |
| **Docker PS** | Currently running Docker containers |
| **Docker PS All** | All containers, including stopped ones |
| **Docker Images** | Docker images available locally |
| **Docker Clean** | Removes stopped containers, unused networks and dangling images |
| **Tail Syslog** | Last 50 lines of the system log |
| **Disk I/O** | Disk input/output statistics |
| **Python Version** | Installed Python 3 and pip versions |
| **Pip Outdated** | Python packages with a newer version available |
| **Node Version** | Installed Node.js and npm versions |
| **NPM Outdated** | npm packages with a newer version (run from a Node project folder) |
...
2. Toggle **Development** off

The category and all its buttons disappear from view. They are not deleted — you can bring them back any time from the same place.
...
!!! note
    Editing default buttons requires [Commandeck Pro](../pro.md). On the free tier, you can use, hide, and delete default buttons, but not edit them.
...
You get **10 free custom buttons**. Here is an easy one to start:

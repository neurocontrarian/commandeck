...
from commandeck_core.utils.sandbox import host_argv, host_env, host_shell_argv, host_which
...
_TERMINAL_CANDIDATES = [
    ...
    ["konsole", "-e"],
    ["ptyxis", "--"],                      # GNOME 48+ / Fedora & recent Ubuntu default
    ["kgx", "-e"],                         # GNOME Console
    ...
    ["foot"],                              # wlroots/Sway Wayland default
    ["alacritty", "-e"],
    ["wezterm", "start", "--"],
    ["kitty"],
    ["terminology", "-e"],                 # Enlightenment
    ["urxvt", "-e"],                       # rxvt-unicode (tiling/minimal setups)
    ["st", "-e"],                          # suckless terminal
    # Debian/Ubuntu/Mint catch-all (alternatives-managed default); per Debian
    # policy it accepts "-e CMD ARGS…". Last so a known terminal above wins first.
    ["x-terminal-emulator", "-e"],
]


@functools.cache
def _resolve_terminal(token: str) -> list[str] | None:
    """Resolve one terminal token (a binary name) to its launch prefix, or None
    if it isn't installed. Uses our known invocation for that binary when we have
    one (so e.g. ptyxis gets ``--`` not ``-e``); otherwise defaults to ``-e``."""
    if not token:
        return None
    binary = token.split()[0]
    if not host_which(binary):
        return None
...
@functools.cache
def detect_terminal(preferred: str = "") -> list[str] | None:
    """Return the prefix command for a terminal emulator, or None.

    Resolution order: the user's ``preferred`` terminal (from Preferences), then
    the ``$TERMINAL`` environment variable, then the first installed terminal in
    ``_TERMINAL_CANDIDATES``. Inside Flatpak, lookup happens on the host (the
    sandbox runtime ships no terminal).
    """
    for token in (preferred, os.environ.get("TERMINAL", "")):
        resolved = _resolve_terminal(token)
...
def available_terminals() -> list[str]:
    """Binary names of known terminal emulators currently installed — for the
    Preferences 'preferred terminal' picker."""
    return [cmd[0] for cmd in _TERMINAL_CANDIDATES if host_which(cmd[0])]


# Default command timeout (seconds) when no setting is injected — shared by the
# executor fallback and the headless MCP executor so they never disagree.
DEFAULT_TIMEOUT = 30
...
def combined_output(result: "ExecutionResult") -> str:
    """stdout + (translated) stderr separator + stderr, or "(no output)" — the single
    formatting used by the desktop output dialog and the mobile output view."""
    ...
    if result.stderr:
        if out:
            out += f"\n{_('--- stderr ---')}\n"
        out += result.stderr
    return out or _("(no output)")


class CommandExecutor:
    def __init__(self, config: ConfigManager,
                 get_timeout: Callable[[], int] | None = None,
                 get_terminal: Callable[[], str] | None = None):
        self._config = config
        self._get_global_timeout = get_timeout
        # Optional callback returning the user's preferred terminal ("" = auto).
    ...
    def _get_timeout(self, button: CommandButton | None = None) -> int:
        ...
        return DEFAULT_TIMEOUT  # no timeout source injected (e.g. MCP / headless)
        ...
        try:
            ...
            if working_dir:
                ...
                if _sys2.platform.startswith("win"):
                    # A double-quote can't legally appear in a Windows path, so strip it:
                    # otherwise a crafted working_dir could close the quoted `cd` and append
                    # its own `&& <command>` (single-user threat model, but cheap to harden).
                    safe_wd = working_dir.replace('"', '')
                    effective_cmd = f'cd /d "{safe_wd}" && {effective_cmd}'
                else:
            ...
            proc = subprocess.run(
                ...
                timeout=timeout,
                input=stdin_input,
                # Strip our bundled LD_LIBRARY_PATH so system binaries (systemctl,
                # w, journalctl, …) load the system's libs, not the AppImage's.
                env=host_env(),
            )
            return ExecutionResult(
    ...
    def _run_in_terminal(self, command: str, button_id: str = "") -> ExecutionResult:
        ...
        preferred = self._get_terminal() if self._get_terminal else ""
        terminal = detect_terminal(preferred)
        if terminal is None:
            return ExecutionResult(
                success=False, return_code=-1,
                stdout="",
        ...
        try:
            ...
            subprocess.Popen(
                ...
                # Same as above: the terminal + the command it runs are system
                # binaries; don't let them see our bundled libraries.
                env=host_env(),
            )
            return ExecutionResult(

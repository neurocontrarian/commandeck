...
class CommandButton:
    ...
    has_sudo_password: bool = False                    # True = a sudo password is stored in the OS keychain (secret_store)
    sudo_password_encoded: str = ""                    # LEGACY: pre-2026-05-29 inline XOR; migrated out by ConfigManager
    timeout: int = 0                                   # Per-button timeout in seconds; 0 = use global GSetting
    os: str = ""                                       # OS the COMMAND targets: ""=cross-platform, "linux"/"macos"/"windows".
                                                       # Drives import dedup (variants coexist) + machine propagation (pair matching OS only).
    source_pack: str = ""                              # Pack id this button came from ("" = custom/seeded); "linux-defaults#2" for duplicates
    pack_button_id: str = ""                            # STABLE per-pack button id (from the pack file); "" = custom/seeded. Update key: matches v1↔v2 buttons on pack update, preserving machine_ids.
    pack_base_command: str = ""                          # The command AS SHIPPED by the pack at the installed version. Lets a pack update tell "user edited the command" (command != base) from "untouched"; "" = legacy/no baseline (refresh silently).

    def _secret_id(self) -> str:
        return f"button:{self.id}"

    def get_sudo_password(self) -> str:
        # Legacy inline secret (only present until ConfigManager migrates it).
        if self.sudo_password_encoded:
        ...
        if self.has_sudo_password:
            from commandeck_core.services.secret_store import get_secret
            return get_secret(self._secret_id())
        return ""

    def set_sudo_password(self, password: str) -> None:
        from commandeck_core.services.secret_store import set_secret, delete_secret
        self.sudo_password_encoded = ""  # never store the secret inline anymore
        if password:
            set_secret(self._secret_id(), password)
            self.has_sudo_password = True
        else:
            delete_secret(self._secret_id())
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'pack_button_id': self.pack_button_id,
            'pack_base_command': self.pack_base_command,
        }
    ...
    def from_dict(cls, data: dict) -> 'CommandButton':
        ...
        return cls(
            ...
            has_sudo_password=data.get('has_sudo_password', False),
            sudo_password_encoded=data.get('sudo_password_encoded', ''),  # legacy → migrated on load
            ...
            pack_button_id=data.get('pack_button_id', ''),
            pack_base_command=data.get('pack_base_command', ''),
        )

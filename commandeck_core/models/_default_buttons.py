"""Default buttons seeded on first run + the installable pack registry.

`get_default_buttons()` seeds the host-OS set at first run (is_default=True,
not editable in free tier). `get_pack(...)` installs an arbitrary pack at
runtime via the Pro "Install Button Pack" dialog — is_default=False (user-
installed), with the pack id stamped onto each button's `source_pack` so we
can detect re-installs and offer Replace vs Add-as-duplicate, and the pack's
OS stamped onto `os` (drives import dedup + machine pairing).
"""
...
def get_default_buttons(target_os: str | None = None) -> list[CommandButton]:
    ...
    if plat.startswith("win"):
        ...
        os_tag = "macos" if plat.startswith(("darwin", "mac")) else "linux"
...
class PackDef:
    """Metadata for an installable pack (declarative — the builder is the source of truth for contents)."""
    id: str                                   # stable key, also goes into button.source_pack
    name_key: str                             # translation key (passed through _())
    description_key: str                      # translation key (passed through _())
    os: str                                   # OS the pack's commands are written for → stamped on button.os
    builder: Callable[[], list[CommandButton]]


def _linux_pack() -> list[CommandButton]:
    return _linux_essentials() + _linux_development()


def _windows_pack() -> list[CommandButton]:
    return _windows_essentials() + _windows_development()


# v1: macOS aliases Linux — bash commands are identical (same choice as
# get_default_buttons, which serves macOS the POSIX set). Mac-specific content
# (brew status, Time Machine, etc.) is a follow-up design task.
PACKS: dict[str, PackDef] = {
    "linux-defaults": PackDef(
        id="linux-defaults",
        name_key="Linux defaults",
        description_key="System, network and dev commands using bash. Best for Linux targets.",
        os="linux",
        builder=_linux_pack,
    ),
    "macos-defaults": PackDef(
        id="macos-defaults",
        name_key="macOS defaults",
        description_key="Same commands as Linux defaults — bash works on macOS targets too.",
        os="macos",
        builder=_linux_pack,
    ),
    "windows-defaults": PackDef(
        id="windows-defaults",
        name_key="Windows defaults",
        description_key="System, network and dev commands using PowerShell. Best for Windows targets.",
        os="windows",
        builder=_windows_pack,
    ),
}


def get_pack(pack_id: str, machine_ids: list[str], category: str,
             source_pack_marker: str | None = None) -> list[CommandButton]:
    """Build a fresh batch of buttons from a pack, targeting the given machines.

    `source_pack_marker` defaults to `pack_id`; pass `"linux-defaults#2"` etc.
    when adding-as-duplicate. Returned buttons are NOT defaults (`is_default=False`),
    positions stay at 0 — ConfigManager.save_buttons reassigns them sequentially.
    Each button is stamped with the pack's `os` (provenance of the command set).
    """
    if pack_id not in PACKS:
        raise KeyError(f"Unknown pack id: {pack_id!r}")
    pack = PACKS[pack_id]
    marker = source_pack_marker if source_pack_marker is not None else pack_id
    buttons = pack.builder()
    for btn in buttons:
        btn.machine_ids = list(machine_ids)
        btn.category = category
        btn.source_pack = marker
        btn.os = pack.os
...
def _linux_development() -> list[CommandButton]:
    return [
        ...
        CommandButton(
            ...
            color="#e5a50a",
            show_output=True,
            # `pip list --outdated` queries PyPI for every installed package, which
            # routinely exceeds the 30s default. Give it room.
            timeout=120,

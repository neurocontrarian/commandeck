...
Commandeck is an app for **Windows, macOS, Linux and Android** that turns commands into
...
![Platforms](https://img.shields.io/badge/platforms-Linux%20%C2%B7%20macOS%20%C2%B7%20Windows%20%C2%B7%20Android-555)
![Private by design](https://img.shields.io/badge/private-no%20account%20%C2%B7%20no%20cloud-2ea44f)
...
- A native **Android app** lets you run the same buttons from your phone
...
Commandeck runs entirely on your own devices. **No account, no cloud, no telemetry —
nothing leaves your machine.** Your buttons are plain text files in your own config
folder, and your commands are yours alone. There's no server in the middle and nothing
to sign up for.
...
The local tier is **free** — unlimited local commands and buttons, no account, no
telemetry, no expiry.
...
Commandeck ships with ready-made buttons, and an in-app gallery offers more — **free,
signed button packs** for common setups (hardware checks, networking, dev tools, a
home-server starter kit, and more). Install, update or remove them in a couple of clicks.

**Have a command worth sharing?** Community packs are welcome — see
[commandeck.app](https://commandeck.app) for how to submit one.
...
A native Android app brings the same buttons to your phone. See
[commandeck.app](https://commandeck.app) for availability.
...
- **SSH** — via Paramiko; authenticate by key (sudo passwords kept encoded, never in plaintext or backups)
...
Desktop, Cursor…) can read, create, and run your buttons. 23 tools. Disabled by
...
Commandeck is **proprietary software**. The local tier is free to use (freeware); Pro
unlocks the server and AI features. Your data stays on your device — no account, no cloud.

See [LICENSE](LICENSE) for the End User License Agreement and
[THIRD_PARTY_LICENSES](THIRD_PARTY_LICENSES.md) for bundled open-source components and
their notices. The **Commandeck** name and logo are trademarks of the author.

© 2026 Neurocontrarian. All rights reserved.
